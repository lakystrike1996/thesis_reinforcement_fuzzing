...
class PolicyGradientModel(tf.keras.Model):
    def __init__(self, num_inputs, num_outputs, num_layers):
        ...
        layers += [tf.keras.layers.Dense(num_outputs, activation=tf.nn.softplus, kernel_initializer=tf.keras.initializers.Constant(value=0.5))]
...
def get_heatmap(input, n_actions):
    ...
    y = np.squeeze(pg_model(np.atleast_2d(x)))
    tot = np.sum(y)
    probs = y / tot
...
def get_loss():
    ...
    for i in range(0, memory.count):
        ...
        entropy = -tf.reduce_sum(log_policy*probs1, axis=-1)
