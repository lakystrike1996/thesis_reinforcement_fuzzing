...
int piece_insert_mutation(afl_state_t* afl, u8** out_buf, u8* in_buf, int pos, bool ex, u32 old_len, u32* new_len) {
  ...
  log_message_int("piece_len: %d\n", piece_len);
  log_message_int("from: %d\n", from);
  //u64 orig_hit_cnt = afl->queued_paths + afl->unique_crashes;
...
int clone_piece_insert_mutation(afl_state_t* afl, u8** out_buf, u8* in_buf, int pos, bool ex, u32 old_len, u32* new_len) {
  ...
  log_message_int("clone_len: %d\n", clone_len);
  log_message_int("clone_from: %d\n", clone_from);
...
int auto_extra_overwrite_mutation(afl_state_t* afl, u8** out_buf, u8* in_buf, int pos, bool ex, u32 old_len, u32* new_len) {
  ...
  log_message_int("a_extras_count=%d\n", afl->a_extras_cnt);
  log_message_int("k=%d\n", k);
  log_message_int("len=%d\n", afl->a_extras[k].len);
...
void send_seed_and_size(char* seed, long length) {
    ...
    sem_post(out_sem);
    log_message_long("SENT seed and size=%ld\n", length);
    sem_wait(in_sem);
    log_message_str("RECEIVED: %s\n", in_buf_shmem);    
...
long get_next_action() {
    ...
    log_message_long("RECEIVED next_action=%ld\n", next_pos); 
...
void send_reward(long reward) {
    ...
    sem_post(out_sem);
    log_message_long("SENT reward=%ld\n", reward);
    sem_wait(in_sem);
    log_message_str("RECEIVED: %s\n", in_buf_shmem);    
