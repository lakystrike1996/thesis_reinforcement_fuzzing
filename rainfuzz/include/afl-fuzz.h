...
char *in_buf_shmem, *out_buf_shmem;
sem_t *out_sem, *in_sem;
...
static inline void log_time(char* format_string) {
#ifdef RAIN_LOG_TIMES
  struct timeval current_time;
  gettimeofday(&current_time, NULL);
  fprintf(log_file, format_string, (current_time.tv_sec * 1000000) + current_time.tv_usec);
#endif
}

static inline void log_message_str(char* format_string, char* argument) {
#ifdef RAIN_LOGS
  fprintf(log_file, format_string, argument);
  fflush(log_file);
#endif
}

static inline void log_message_int(char* format_string, int argument) {
#ifdef RAIN_LOGS
  fprintf(log_file, format_string, argument);
  fflush(log_file);
#endif
...
static inline void log_message_long(char* format_string, long argument) {
#ifdef RAIN_LOGS
  fprintf(log_file, format_string, argument);
  fflush(log_file);
#endif
...
static inline void log_message(char* format_string) {
#ifdef RAIN_LOGS
  fprintf(log_file, format_string);
  fflush(log_file);
#endif
}

static inline void log_seed(char* message, u8* buf, u32 len) {
#ifdef RAIN_LOG_SEEDS
  ...
  fflush(log_file); //TODO: togliere fflush ovunque
#endif
}
...
struct queue_entry {
  ...
  u8* original_coverage;                /* Coverage map of this seed        */
...
#define FOREIGN_SYNCS_MAX 32U
  ...
  u8 last_exec_new_cov;
  
  u64 fuzz_one_original_count;
